...
RemoteScreensService::RemoteScreensService(QObject *parent)
    : QObject(parent)
{
    // "/" -> redirect.
    m_httpServer.route("/", [](const QHttpServerRequest &req) {
        ...
        QHttpHeaders h;
        h.replaceOrAppend(QHttpHeaders::WellKnownHeader::Location, "https://subtivals.org");
...
RemoteScreensService::~RemoteScreensService() {
    stop();
}

void RemoteScreensService::loadSettingsAndMaybeStart() {
    ...
    m_config.webSocketPort = static_cast<quint16>(s.value("webSocketPort", 8765).toInt());

    // Persist UUID immediately if missing/empty so it survives even when not enabled
    ...
    emit settingsLoaded(m_config);

    qDebug() << "RemoteScreensService loaded settings:" << m_config.enabled << m_config.httpPort << m_config.webSocketPort << m_config.uuid;
    if (m_config.enabled)
        start(m_config.httpPort, m_config.webSocketPort);
}

void RemoteScreensService::saveSettings() const {
    QSettings s("Remote", "RemoteApp");
    s.beginGroup("RemoteScreens");
    ...
    s.setValue("uuid",           m_config.uuid);
    s.endGroup();
    qDebug() << "RemoteScreensService saved settings:" << m_config.enabled << m_config.httpPort << m_config.webSocketPort << m_config.uuid;
}

void RemoteScreensService::start(int httpPort, int webSocketPort) {
    if (m_isRunning)
        stop();

    qDebug() << "Starting RemoteScreensService with config:" << httpPort << webSocketPort << m_config.uuid;
    ...
    if (!m_tcpServer->listen(QHostAddress::Any, m_config.httpPort)) {
        if (!m_tcpServer->listen(QHostAddress::Any, 0)) {
            emit errorOccurred(QStringLiteral("Cannot listen HTTP on port %1").arg(m_config.httpPort));
            m_tcpServer.reset();
            return;
        }
    }
    if (!m_httpServer.bind(m_tcpServer.get())) {
        ...
        m_tcpServer->close();
        m_tcpServer.reset();
        emit errorOccurred(QStringLiteral("QHttpServer bind() failed on port %1").arg(p));
        return;
    }
    m_httpPortInUse = m_tcpServer->serverPort();

    // WebSocket
    m_webSocketServer = std::make_unique<QWebSocketServer>("RemoteScreensWS",
    ...
    if (!m_webSocketServer->listen(QHostAddress::Any, m_config.webSocketPort)) {
        if (!m_webSocketServer->listen(QHostAddress::Any, 0)) {
            ...
            if (m_tcpServer) { m_tcpServer->close(); m_tcpServer.reset(); }
            emit errorOccurred(QStringLiteral("Cannot bind WebSocket server to port %1").arg(m_config.webSocketPort));
            return;
        }
    }
    m_webSocketPortInUse = m_webSocketServer->serverPort();

    connect(m_webSocketServer.get(), &QWebSocketServer::newConnection, this, [this]() {
    ...
    connect(socket, &QWebSocket::textMessageReceived, this, [this, socket](const QString &msg) {
        ...
        if (!authorized) {
            ...
            const QJsonDocument doc = QJsonDocument::fromJson(msg.toUtf8(), &parseErr);
            ...
            // (Optional) send an OK event; comment out if you prefer silence
            const QJsonObject ok{{"event-type","ok"},{"data","connected"}};
            socket->sendTextMessage(QString::fromUtf8(QJsonDocument(ok).toJson(QJsonDocument::Compact)));
            return;
        }
...
void RemoteScreensService::stop() {
    qDebug() << "Stopping RemoteScreensService";
    unpublishMdns();
...
void RemoteScreensService::disable() {
...
QString RemoteScreensService::effectiveHost() const {
...
QString RemoteScreensService::buildViewerUrl(const QString &host) const {
...
void RemoteScreensService::sendMessage(const QString &message) {
    ...
    const QString encoded = QString::fromUtf8(QJsonDocument(ok).toJson(QJsonDocument::Compact));

    const auto clients = m_clients;
    for (QWebSocket* socket : clients) {
        if (!socket) continue;
        if (!socket->isValid()) continue;
        if (!socket->property("authorized").toBool()) continue;

        socket->sendTextMessage(encoded);
    }
}

void RemoteScreensService::publishMdns() {
#ifdef Q_OS_MAC
    unpublishMdns();
    qDebug() << "Publishing mDNS service";
    m_dnsSdProcess = new QProcess(this);
    const QString name = QStringLiteral("RemoteScreens-%1").arg(m_config.uuid.left(8));
    m_dnsSdProcess->start("dns-sd", {"-R", name, "_http._tcp", ".", QString::number(m_httpPortInUse)});
#endif
}

void RemoteScreensService::unpublishMdns() {
#ifdef Q_OS_MAC
